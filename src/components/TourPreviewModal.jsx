...
const TourPreviewModal = ({ tour, isOpen, onClose, onAddToCart, loading = false }) => {
    ...
    >
      ...
      >
        ...
        <div className="p-3 sm:p-6 md:p-8 space-y-4 sm:space-y-7">
          ...
          {/* Footer - Updated with Add to Cart */}
          <div className="border-t border-slate-200 pt-3 sm:pt-6 flex gap-2 sm:gap-4">
            <Link
              to={`/tour-details/${tour.id}`}
              onClick={onClose}
              className="flex-1 border-2 border-primary text-primary font-semibold sm:font-bold py-2 sm:py-3 rounded-lg hover:bg-primary/5 transition text-xs sm:text-base text-center"
            >
              View Details
            </Link>
            <button
              onClick={onAddToCart}
              disabled={loading}
              className={`flex-1 bg-primary text-white font-semibold sm:font-bold py-2 sm:py-3 rounded-lg transition text-xs sm:text-base flex items-center justify-center gap-2 shadow-lg shadow-primary/30 hover:bg-primary-dark ${
                loading ? "opacity-60 cursor-not-allowed" : ""
              }`}
            >
              <span className="material-icons text-base">
                {loading ? "hourglass_empty" : "add_shopping_cart"}
              </span>
